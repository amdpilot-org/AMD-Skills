...
- **aiter Triton GEMM default configs suboptimal for decode**: The JSON config files at `aiter/ops/triton/configs/gemm/gfx950-*.json` ship with large `BLOCK_SIZE_M` (128-256) and `NUM_KSPLIT=1` in the `"any"` fallback entry — tuned for training, terrible for batch=1 decode. Add `M_LEQ_4/8/16` entries with small `BLOCK_SIZE_M` and `NUM_KSPLIT > 1` for K-split parallelism. This is a simple JSON edit that can reduce decode latency 30-50%. Details: [references/gemm-and-linear.md](references/gemm-and-linear.md).
